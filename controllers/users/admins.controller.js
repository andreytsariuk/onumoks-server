...
const { Admins } = require("../../db/models").RolesTypes;
...
module.exports = class {
  static list(req, res, next) {
    return RequireFilter.Check(req.body, requireFields.List)
      .then(validated => new Admins())
        ...
        },
        {
          withRelated: [
            "roles",
            {
              "roles.user": qb => {
                if (req.params.search && req.params.search !== "undefined")
                  qb.whereRaw(`LOWER(email) LIKE ?`, [
                    `%${_.toLower(req.params.search)}%`
                  ]);
              },
              "roles.user.profile": qb => {
                if (req.params.search && req.params.search !== "undefined") {
                  qb.whereRaw(`LOWER(fname) LIKE ?`, [
                    `%${_.toLower(req.params.search)}%`
                  ]);
                  qb.whereRaw(`LOWER(lname) LIKE ?`, [
                    `%${_.toLower(req.params.search)}%`
                  ]);
                }
              }
  ...
  static create(req, res, next) {
    return RequireFilter.Check(req.body, requireFields.Create)
